export const Akad = () => {
  return (
    <section>
      <div className="container mx-auto pt-4 pb-20">
        <div className="mx-52 flex flex-row justify-around">
          <div className="text-center">
            <h1 className="satin-secondary text-5xl font-bold text-satin-900">Akad</h1>
            <p className="mt-2 mb-6 text-xl">Sabtu, 6 Mei 2023</p>
            <p className="mb-4 text-lg">
              RDM Garage, <br /> Somosari 02/02 Batealit Jepara <br /> <span className="font-bold"> 09.00 WIB - Selesai</span>
            </p>
            <a
              href="https://goo.gl/maps/qU6yrahz7C5yJdgq8"
              role="button"
              target="_blank"
              rel="noopener noreferrer"
              className="mt-8 mr-2 mb-2 inline-flex items-center rounded-full bg-[#3F3C38] px-7 py-2.5 text-center font-medium text-white hover:bg-[#3F3C38]/90 focus:outline-none focus:ring-4 focus:ring-[#3F3C38]/50">
              Lihat Lokasi
            </a>
          </div>
          <div className="text-center">
            <h1 className="satin-secondary text-5xl font-bold text-satin-900">Resepsi</h1>
            <p className="mt-2 mb-6 text-xl">Sabtu, 6 Mei 2023</p>
            <p className="mb-4 text-lg">
              RDM Garage, <br /> Somosari 02/02 Batealit Jepara <br /> <span className="font-bold"> 13.00 WIB - 18.00 WIB</span>
            </p>
            <a
              href="https://goo.gl/maps/qU6yrahz7C5yJdgq8"
              role="button"
              target="_blank"
              rel="noopener noreferrer"
              className="mt-8 mr-2 mb-2 inline-flex items-center rounded-full bg-[#3F3C38] px-7 py-2.5 text-center font-medium text-white hover:bg-[#3F3C38]/90 focus:outline-none focus:ring-4 focus:ring-[#3F3C38]/50">
              Lihat Lokasi
            </a>
          </div>
        </div>
      </div>
    </section>
  )
}
